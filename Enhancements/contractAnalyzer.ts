...
import { detectPrimaryLanguage, getLanguageDisplayName, isEvmCompatible } from "@/utils/languageDetection";
import { getLanguageSpecificPrompt } from "./languagePrompts";
import { advancedFlashLoanDetector } from "./modules/advancedFlashLoanDetector";
import { solanaAdvancedDetector } from "./modules/solanaAdvancedDetector";
import { moveAdvancedDetector } from "./modules/moveAdvancedDetector";
import { cairoAdvancedDetector } from "./modules/cairoAdvancedDetector";
import { economicImpactAnalyzer } from "./modules/economicImpactAnalyzer";
import { dataFlowAnalyzer, taintAnalyzer } from "./modules/dataFlowAnalyzer";
...
export async function analyzeContract(params: {
  ...
  while (retryCount < maxRetries) {
    try {
      ...
      // Detect the programming language from the files
      const detectedLanguage = detectPrimaryLanguage(filesToAnalyze);
      const languageName = getLanguageDisplayName(detectedLanguage);
      
      logger.info('Language Detection', `Detected language: ${languageName}`);

      // Check if this is a Monad deployment (EVM-compatible but optimized for high throughput)
      const isMonad = params.chain?.toLowerCase().includes('monad') || false;
      
      // Get language-specific prompt if available
      const languageSpecificPrompt = getLanguageSpecificPrompt(
        detectedLanguage,
        mergedCode,
        params.contractName || '',
        isMonad
      ...
      // Use language-specific prompt or fall back to enhanced Solidity prompt
      let basePrompt = languageSpecificPrompt || 
        ENHANCED_ENHANCED_SECURITY_AUDIT_PROMPT.replace("${mergedCode}", mergedCode).replace(
          "${params.contractName ? params.contractName : ''}",
          params.contractName || ""
        );

      let finalPrompt = createPromptWithLanguage(basePrompt, config.language);
      ...
      // Add language detection info at the top
      const languageNote = isMonad 
        ? `> **Detected Language:** ${languageName} (Monad/EVM-compatible)\n> **Note:** Monad is fully EVM-compatible and uses Solidity. Analysis includes considerations for high-throughput parallel execution.\n\n`
      ...
      if (isEvmCompatible(detectedLanguage)) {
        ...
        logger.info('Vulnerability Detection', `Found ${economicFindings.length} potential EVM-specific vulnerabilities`);
      } else {
